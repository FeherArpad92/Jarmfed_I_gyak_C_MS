...
uint8_t szam1=0, szam2=0, muvelet=1;
/******************************************************************************
* External Variables
******************************************************************************/
...
{
	DDRA = 0xff; //eredm?ny
	DDRF = (1<<PF3) | (1<<PF2) | (1<<PF1) | (1<<PF0); // timer m?k?d?s?t jelzi
	PORTF = (1<<PF3) | (1<<PF2) | (1<<PF1) | (1<<PF0);
	
	DDRB = (0<<PB4) | (0<<PB3) | (0<<PB2) | (0<<PB1) | (0<<PB0); // als? 5 bit bemenet
	PORTB = (1<<PB4) | (1<<PB3) | (1<<PB2) | (1<<PB1) | (1<<PB0); // felh?z? ellenŠsok bekapcsolŠsa
	
	DDRD = 0xff; // szŠmok m?veletekhez
	DDRE = 0xff; // m?veleti jel
	...
	while(1)
	{
		...
		if(timer_task_10ms)
		{
			...
			switch (muvelet)
			{
				...
				case OSZTAS:
				{
					if(szam2!=0) PORTA = szam1/szam2; else PORTA =0;
					break;
				}
			}
